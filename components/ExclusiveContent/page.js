...
import { FaFacebook, FaTwitter, FaTimes, FaLinkedin } from "react-icons/fa";
...
const ExclusiveContent = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${baseUrl}/api/news/type/breaking`);
  ...
  const renderSkeleton = (count, type) => {
    return Array.from({ length: count }).map((_, index) => {
      if (type === "highlight") {
        return (
          <div
            key={index}
            className="relative group overflow-hidden shadow-lg cursor-pointer bg-gray-200 animate-pulse h-80"
          ></div>
        );
      } else {
        return (
          <div
            key={index}
            className="bg-gray-100 p-4 rounded-lg shadow-sm flex flex-col-reverse md:flex-row lg:flex-row gap-2 cursor-pointer animate-pulse"
          >
            <div className="w-full md:w-[100px] lg:w-[100px] h-[100px] bg-gray-300"></div>
    ...
    <div className="bg-white py-8 px-10">
      <div className="container mx-auto">
        ...
        <div className="flex justify-between items-center mb-6 border-b pb-4">
          <h2 className="text-[13px] text-center hidden md:block lg:block">
            Join or social media
          </h2>
          <h2 className="text-base lg:text-lg md:text-lg font-bold text-center">
            For even more exclusive content!
          </h2>
          <div className="lg:flex md:flex items-center space-x-4 hidden ">
            <Link href="https://www.facebook.com/textiletrend4u?mibextid=ZbWKwL">
              <FaFacebook className="text-red-500 cursor-pointer" size={20} />
            </Link>
            <Link href="https://www.linkedin.com/company/textile-trend/posts/?feedView=all">
              <FaLinkedin className="text-red-500 cursor-pointer" size={20} />
            </Link>
            ...
            : breaking.slice(4, 12).map((article) => (
              ...
              >
                <h3 className="text-sm font-bold mt-2">
                  <span className="text-white bg-red-500 px-3 py-1 rounded self-start mr-2 mb-2  text-xs font-bold uppercase">
                    {article.category}
                  </span>
                  {article.title}
                </h3>
